...
import { getContabilidadSyncState } from "@/lib/contabilidadSyncStatus";
...
export function AdminOrderDetailDrawer() {
  ...
  const retry = trpc.admin.orders.retryContabilidad.useMutation({
    onSuccess: async result => {
      if (result.status === "synchronized") toast.success("Venta registrada en Contabilidad.");
      else toast.error(`No se pudo completar el registro: ${result.reason ?? "revisa la configuración."}`);
      await Promise.all([orders.refetch(), events.refetch()]);
    },
    onError: error => toast.error(error.message),
  });
  const latestSync = events.data?.find(event => event.eventType === "contabilidad_sale") ?? order?.contabilidadSync ?? null;
  const syncState = getContabilidadSyncState(order?.status ?? "pending", latestSync);
  const canRetry = syncState.retryAllowed;
