...
export async function createPurchaseTicketPdf(input: PurchaseTicketInput): Promise<Buffer> {
  ...
  doc.fillColor("#6b7280").font("Helvetica").fontSize(8).text(input.paymentId ? `Referencia Mercado Pago: ${input.paymentId}` : "", 48, 735).text("Consulta los Términos y Condiciones de zRabbit para conocer las condiciones de compra y envío.", 48, 748, { width: 499 });
