...
export async function createPendingOrder(input: { customerName: string; customerEmail: string; customerPhone?: string; shippingAddress?: string; shippingDistrict?: string; shippingAgencyName?: string; shippingAgencyAddress?: string; items: { productId: number; quantity: number }[] }) {
  const db = await getDb();
  ...
  await notifyOrderCreated({ orderNumber, totalInCents, shippingMethod: isYapeTestOnly ? "yape_test" : "shalom", customerName: input.customerName, customerPhone: input.customerPhone, shippingAddress: input.shippingAddress, shippingDistrict: input.shippingDistrict, shippingAgencyName: input.shippingAgencyName, shippingAgencyAddress: input.shippingAgencyAddress, items: enriched.map(({ product, quantity }) => ({ productName: product.name, quantity })) });
